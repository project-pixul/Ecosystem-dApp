...
import { Pixul } from '../../contracts/pixul';
import {
	WithTranslation,
	withTranslation,
	TFunction,
...
> {
	...
	async confirmStake(): Promise<void> {
		try {
			...
			if (state.ctValueStake >= 0) {
				await state.pixul.stake(state.ctValueStake);
			} else {
	...
	async confirmUnstake(): Promise<void> {
		try {
			...
			if (state.ctValueUnstake >= 0) {
				await state.pixul.unstakeAndClaim(state.ctValueUnstake);
			} else {
	...
	componentWillUnmount() {
		...
		this.updateState({ pixul: null, looping: false });
	}
	...
	private async updateOnce(resetCt?: boolean): Promise<boolean> {
		const pixul = this.readState().pixul;
		...
		if (!!pixul) {
			try {
				...
				this.updateState({
					address: pixul.wallet.currentAddress,
					balance: pixul.balance,
					stakedBalance: pixul.stakedBalance,
					pendingRewards: pixul.pendingStakeRewards,
				});
				...
				<div className='container'>
					<div className='row text-white staking-header'>
						<div className='col-md-12'>
							...
							<p>
								<Trans i18nKey='staking.paragraph2'>
									In order to stake xPixul tokens, you need to connect your
									browser wallet (such as{' '}
											...
											<form id='staking-form'>
												...
												<div className='d-flex flex-row align-items-baseline staking-slider-wrapper'>
													<input
														...
														step='1'
														disabled={state.pending}
														value={state.ctPercentageStake || 0}
														onChange={this.handleStakeSlider}
														style={{ border: 'none', background: 'none' }}
													/>
													<label className='form-label align-self-center'>
														{numeral(state.ctPercentageStake || 0).format('0')}%
													</label>
												</div>
												<div className='d-flex flex-row justify-content-evenly staking-percentages'>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setStakePercentage(0)}>
														0%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setStakePercentage(25)}>
														25%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setStakePercentage(50)}>
														50%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setStakePercentage(75)}>
														75%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setStakePercentage(100)}>
														100%
													</button>
												</div>
												<label className='form-label'>
												...
												<input
													...
													disabled={state.pending}
													onChange={this.handleInputStake}
													value={state.ctValueStake || 0}
												/>
												<div className='d-flex justify-content-center button-row'>
													<button
														...
														disabled={state.ctValueStake <= 0 || state.pending}
														type='button'
														onClick={async () => this.confirmStake()}>
														{t('staking.stake.title')}
													</button>
													<button
														className='btn btn-complementary btn-md link-dark align-self-center stake-claim'
														disabled={state.pendingRewards <= 0}
														type='button'
														onClick={async () => this.confirmClaimRewards()}>
														{t('staking.stake.claim_rewards')}
													</button>
												</div>
											</form>
										</div>
											...
											<form id='unstaking-form'>
												...
												<div className='d-flex flex-row align-items-baseline staking-slider-wrapper'>
													<input
														...
														step='1'
														disabled={state.pending}
														value={state.ctPercentageUnstake || 0}
														onChange={this.handleUnstakeSlider}
														style={{ border: 'none', background: 'none' }}
													/>
													<label className='form-label align-self-center'>
														{numeral(state.ctPercentageUnstake || 0).format(
															'0',
														)}
														%
													</label>
												...
												<div className='d-flex flex-row justify-content-evenly staking-percentages'>
													<button
														...
														type='button'
														disabled={state.pending}
														onClick={() => this.setUnstakePercentage(0)}>
														0%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setUnstakePercentage(25)}>
														25%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setUnstakePercentage(50)}>
														50%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setUnstakePercentage(75)}>
														75%
													</button>
													<button
														className='btn btn-dark btn-sm flex-grow-1 flex-shrink-0 flex-fill'
														type='button'
														disabled={state.pending}
														onClick={() => this.setUnstakePercentage(100)}>
														100%
													</button>
												</div>
												<label className='form-label'>
													{t('staking.unstake.amount')}
												</label>
												<input
													type='number'
													className='form-control form-control-lg'
													disabled={state.pending}
													onChange={this.handleInputUnstake}
													value={state.ctValueUnstake || 0}
												/>
												<div className='d-flex justify-content-center button-row'>
													<button
														className='btn btn-primary btn-md link-dark align-self-center stake-confirm'
														disabled={
															state.ctValueUnstake <= 0 || state.pending
														}
														type='button'
														onClick={async () => this.confirmUnstake()}>
														{t('staking.unstake.title')}
													</button>
													<button
														className='btn btn-complementary btn-md link-dark align-self-center stake-claim'
...
export default withTranslation()(TransactionComponent);
